...
const { ensure } = require('../config/authGuard');
// Utilities
...
router.post(
  ...
  (req, res, next) => {
    ...
    res.status(403).json({
      status: 403,
      msg: 'You need admin privileges to make this request!',
    });
  },
  (req, res) => {
    if (
      ...
      const medicine = new Medicine({
        name: capitalise(req.body.name).trim(),
        formula: capitalise(req.body.formula).trim(),
